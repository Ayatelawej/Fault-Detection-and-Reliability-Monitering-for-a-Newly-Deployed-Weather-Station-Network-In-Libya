...
OUTPUT_TXT = OUTPUT_DIR / "local_cooccurrence_clean.txt"
OUTPUT_CSV = OUTPUT_DIR / "local_cooccurrence_clean.csv"
...
def build_excluded_hours(windows: pd.DataFrame) -> set:
    excluded: set = set()
    for _, w in windows.iterrows():
        start = pd.Timestamp(w["window_start_utc"], tz="UTC")
        end = pd.Timestamp(w["window_end_utc"], tz="UTC")
        backfill = pd.Timestamp(w["backfill_start_utc"], tz="UTC")
        range_start = backfill
        range_end = end
        current = range_start.floor("h")
...
def main() -> None:
    ...
    excluded_hours = build_excluded_hours(windows)
    ...
    clean_states = active_states[
        ~active_states["hour_utc"].isin(excluded_hours)
    ].copy()
    ...
    lines: list[str] = []
    lines.append(
        "=== Local outage co-occurrence (excluding network-wide window hours) ==="
    )
    lines.append(f"Excluded hours (across network-wide windows): {len(excluded_hours)}")
