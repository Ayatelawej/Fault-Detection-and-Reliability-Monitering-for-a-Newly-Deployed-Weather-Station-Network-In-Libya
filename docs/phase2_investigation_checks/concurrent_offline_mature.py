...
OUTPUT_PATH = Path(__file__).parent / "concurrent_offline_mature.txt"
...
def main() -> None:
    ...
    per_hour = per_hour[per_hour["active_count"] >= MIN_ACTIVE_STATIONS].copy()
    per_hour["offline_fraction"] = (
        per_hour["offline_count"] / per_hour["active_count"]
    )
    per_hour["hour_of_day"] = per_hour.index.hour
    per_hour["year_month"] = per_hour.index.strftime("%Y-%m")
    ...
    ym_dist_high = high_blackout["year_month"].value_counts().sort_index()

    full_blackout = per_hour[per_hour["offline_fraction"] >= 1.0]
    hod_dist_full = full_blackout["hour_of_day"].value_counts().sort_index()
    ym_dist_full = full_blackout["year_month"].value_counts().sort_index()
    ...
    lines: list[str] = []
    lines.append(
        f"=== Concurrent offline analysis (mature network: >= {MIN_ACTIVE_STATIONS} active stations) ==="
    )
    lines.append(f"Total hours analyzed: {total_hours}")
    if total_hours > 0:
        lines.append(
            f"Date range covered: {per_hour.index.min()} to {per_hour.index.max()}"
        )
    lines.append("")
    ...
    for t, c in threshold_counts.items():
        ...
        lines.append(
            f"  >= {int(t * 100):>3d}%: {c:>5d} hours ({pct:.2f}% of all hours)"
        )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "=== Year-month distribution for hours with offline_fraction >= 80% ==="
    )
    lines.append(ym_dist_high.to_string())
    lines.append("")
    lines.append(
        "=== Year-month distribution for hours with offline_fraction == 100% ==="
    )
    lines.append(ym_dist_full.to_string())
    lines.append("")
    ...
    if target_row is not None:
        ...
        lines.append("Target hour not present in filtered dataset.")
